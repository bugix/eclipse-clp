...
#
#
# $Id: eclipse_tools.tcl,v 1.46 2017/10/10 15:50:35 jschimpf Exp $
#
...
set tkecl(version) 7.2 ;# update also in tkeclipse and examples!
...
#	pack $ec_tracertab.trace.label -side left -expand 1 -fill x
	...
	bind $ec_tracer.buttons <Key-e> {tkecl:set_tracercommand e}
...
#	bind $ec_tracer.buttons <Key-minus> {tkecl:set_tracercommand -}
	...
	button $ec_tracer.cont.toerr -text "To Error" -underline 3 -command {tkecl:set_tracercommand e}
	    pack $ec_tracer.cont.toerr -side left
...
#--------------------------------------------------------------------
 ...
 information (left) part of\n the stack item to show source contxt\n "
   ...
   \nKeyboard shortcut: `p'"
       balloonhelp $ec_tracer.cont.toerr "Jump to 'leave' port\
   \nKeyboard shortcut: `e'"
...
proc tkecl:handle_tracer_command {} {
	...
	e { ;# zap to $tkecl(zap_port)
	    tkecl:configure_tracer_buttons disabled
            ec_rpcq_check [list configure_prefilter _ _ leave _ dontcare]]\
                    (__()_()) sepia_kernel
            tkecl:send_tracer_command ""
        }
	filter {
...
proc tkecl:configure_tracer_buttons {state} {
    ...
    $ec_tracer.cont.toerr configure -state $state
...
# Get saved preference values from rc file or registry.
# Return list of option names found, and their values in tkecl(prefset,option).
...
proc tkecl:set_tools_defaults {} {
	...
	foreach {dname default type family help} $preference {
		...
		tkecl:check_and_set_pref $dname $value $type
	    }
	} 
    }
}
	
# Check and set the named preference to the given value.
# If the type check fails, warn and do nothing.
# Otherwise set tkecl(pref,option) and perform any necessary side effect.
# For some options, the empty string is accepted but not applied,
# in order to allow preserving an unknown default setting.
proc tkecl:check_and_set_pref {dname dvalue type} {
    global tkecl
    ...
    switch -- $type {
        ...
        +integer_opt {
            ;# straight positive integers or empty
            if {![regexp {^([0-9]+| *)$} $dvalue]} {
        ...
        +integer {
            ;# straight positive integers, no special routines to call
            if {![regexp {^[0-9]+$} $dvalue]} {
                tk_messageBox -icon warning -message "$dvalue is an invalid value for $dname (positive integer expected)" -type ok
                return
            }
        ...
        tracer_prdepth {
            ...
            ec_rpcq [list set_tracer_print_depth $dvalue] (I) tracer_tcl
        }
...
proc tkecl:popup_edit_defaults {} {
	...
	foreach preference $tkecl(preferences) {
	    tkecl:display_pref $edit $preference 
	}
...
# Add an entry widget for the specified preference.
# Take its current value from tkecl(pref,option),
# and reflect changes in tkecl(prefset,option).
proc tkecl:display_pref {w preference} {
    global tkecl
...
# Apply the displayed preference values (i.e. tkecl(prefset,option))
# and make them current in tkecl(pref,option)
proc tkecl:apply_prefs {} {
	...
	tkecl:check_and_set_pref $option $tkecl(prefset,$option) $type
    }
}

# Save the displayed preference values (i.e. tkecl(prefset,option))
# to rc file or registry, unless they are empty strings.
	...
	windows {
		...
		foreach option $group($rootname) {
		    if {[string trimleft $tkecl(prefset,$option)] != ""} {
                    ...
                    } elseif {[llength [registry values $regpath $option]]} {
                        registry delete $regpath $option
                    }
		}
	    }
	}
...
# Init the Eclipse part (must be done after ec_init !!!)
    ...
    set tkecl(predpropmodule) [lindex [ec_rpcq_check {get_flag toplevel_module _} (()_)] 2]
