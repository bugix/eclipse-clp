...
typedef struct {
    ...
    */
    int		*matbeg;	/* [mac+1] index into matind/matval for each column */
    int		*matcnt;	/* [mac] non-zeros count for each column */
    int		*matind;	/* [matnz] non-zeros row indicies  */
    double	*matval;	/* [matnz] non-zero coefficients  */
    ...
    int		nsos;		/* number of SOSs in problem */
    int		nsos_added;	/* number of SOSs already in solver */
    /* following group only used for buffering during adding new SOSs: */
    int		sossz;		/* size of sos arrays */
    int		sosnzsz;	/* size of sos member arrays */
    int		nsosnz;		/* total number of sos members in arrays */
    sostype_t	*sostype;	/* [sosnz] sos type 1 or 2 */
    int		*sosbeg;	/* [sossz] index into sosind[] */
    int		*sosind;	/* [sosnzsz] col-no */
    double	*sosref;	/* [sosnzsz] weight */
    ...
    struct lp_sol sol;          /* pointers to solution vectors,
                                 * only valid within p_cpx_optimise() */
...
#ifdef GLPK
    glp_env	params;         /* per-problem optimizer parameters */
#endif
} lp_desc;
