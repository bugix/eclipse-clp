...
<P>
   ...
   from within ECLiPSe.  Solver that can be connected through eplex
   include
   <UL>
   <LI>CLP/CBC from the COIN-OR open-source project (eplex_osi)</LI>
   <LI>GLPK (eplex_glpk)</LI>
   <LI>CPLEX (eplex_cplex)</LI>
   <LI>XPRESS-MP (eplex_xpress)</LI>
   <LI>Gurobi (eplex_gurobi)</LI>
   </UL>
   ...
   and does not include the solver or any required licence to use them.
    ...
    <DL>
    <DT>reals(Xs)</DT><DD>the variables Xs all take real values</DD>
    <DT>integers(Xs)</DT><DD>the variables Xs all take integer values</DD>
    <DT>Xs $:: Lwb..Upb</DT><DD>the variables Xs have the given bounds</DD>
...
<DD>Variables. If X is not yet a problem variable for the external solver
    ...
    <TT>X $:: -inf..inf</TT> or <TT>reals([X])</TT>.

<P>
<DT><STRONG>123, 3.4</STRONG>
<DD>Integer or floating point constants.

<P>
<DT><STRONG>+Expr</STRONG>
<DD>Identity.
...
<P>
<DT><STRONG>sum(Vector)</STRONG>
<DD>Equivalent to the sum of all vector elements. The vector can be a list,
array or any of the vector expressions supported by eval_to_list/2.

<P>
<DT><STRONG>sum(Vector1*Vector2)</STRONG>
...
:- comment(integers/1,  [
    ...
    args:      ["Vars":    "Collection or nested collection of variables and numbers"],
...
:- comment((::)/2,  [
    ...
    args: ["Vars":"Variable, number, or (possibly nested) collection of such",
	   "Lo..Hi":"Lo, Hi are numbers or arithmetic expressions"
	],
    see_also:  [($::)/2],
    summary:   "Constrains all collection elements to lie between the given bounds.",
...
:- comment(($::)/2,  [
    ...
    see_also:  [integers/1,reals/1,(#::)/2,_:($::)/2],
    summary:   "Constrains all collection elements to lie between the given bounds.",
    exceptions: [5: "Lo..Hi is not in correct form"],
    desc:      html("\
	...
	given as integers, this information is <STRONG>not</STRONG> passed
...
:-comment(get_changeable_value/2,
          [template:["get_changeable_value(?Var, -Val)","EplexInstance:get_changeable_value(?Var, -Val)"],
           ...
           summary:"Interface predicate to access the `changeable' value for this variable."]).
...
see_also:[($=)/2,($=<)/2,($>=)/2,
          ...
          eplex_solve/1,
          eplex_get/2, eplex_var_get/3, optimize/2, optimize/3
         ],
desc: html("\
<P>
...
<DT><STRONG><TT>method(+Method)</TT></STRONG>
    <DD>Use the specified method (<TT>primal, dual, net, barrier, sifting,
    default, auto, concurrent, concurrent_det</TT>) (representing Primal
    Simplex, Dual Simplex, Network Simplex, Barrier, etc) to solve
    the problem. For MIP problems, this specifies only the start algorithm 
    (the one that is used to solve the initial relaxation). The option
    node_method should be used to specify the method for the other nodes. 
<P> 
...
<P>
    ...
    most solvers:
<DL>
    <DT><TT>feasibility_tol</TT> - float
	<DD>CPX_PARAM_EPRHS (CPLEX), XPRS_FEASTOL (XPRESS-MP),
            OsiPrimalTolerance (OSI), FeasibilityTol (Gurobi) or tol_bnd (GLPK)
    <DT><TT>integrality</TT> - float
	<DD>CPX_PARAM_EPINT (CPLEX), XPRS_MIPTOL (XPRESS-MP),
            CbcIntegerTolerance (OSI,Cbc specific), IntFeasTol (Gurobi)
            or tol_int (GLPK)
    <DT><TT>iteration_limit</TT> - integer
	<DD>CPX_PARAM_ITLIM (CPLEX), XPRS_LPITERLIMIT (XPRESS-MP),
            OsiMaxNumIteration (OSI), IterationLimit (Gurobi) or it_lim (GLPK)
    <DT><TT>mipgap</TT> - float
	<DD>CPX_PARAM_EPGAP (CPLEX), XPRS_MIPRELSTOP (XPRESS-MP),
            SolverAllowableFractionGap (OSI), MIPGap (Gurobi) or mip_gap (GLPK)
    <DT><TT>node_limit</TT> - integer
	<DD>CPX_PARAM_NODELIM (CPLEX), XPRS_MAXNODE (XPRESS-MP),
            CbcMaxNumNode (OSI, Cbc specific), NodeLimit (Gurobi)
            or limit on tree node creation (GLPK)
    <DT><TT>objdifference</TT> - float
	<DD>CPX_PARAM_OBJDIF (CPLEX), XPRS_MIPADDCUTOFF (XPRESS-MP) or
            CbcCutoffIncrement (OSI, Cbc specific)
</DL>
")
]).
...
<DL>
    ...
    <DT><TT>has_sos</TT>
	<DD>Returns 'yes' if the solver supports special ordered sets (SOS),
        i.e. the sos1/1 and sos2/1 constraints.
	else 'no'.
    <DT><TT>optimizer</TT>
	<DD>Returns the name of the external solver, currently
	'cplex', 'glpk', 'gurobi', 'osi' or 'xpress'.
...
:- comment(eplex_get_iis/4, [
        ...
        exceptions: [141: "External solver does not support finding IIS",
                     3: "Error in external solver while getting IIS"
                    ],
...
:- comment(optimize/3, [
    template:["optimize(+Objective, -Cost, +Options)","EplexInstance:optimize(+Objective, -Cost, +Options)"],
    args:["Objective":"Objective function: min(CostExpr) or max(CostExpr)",
        "Options":"List of solver options",
        "Cost":"Output: cost of returned solution"],
    summary: "Setup problem, solve and instantiate problem variables",
    see_also:[($=)/2,($=<)/2,($>=)/2,($::)/2,integers/1,reals/1,
          eplex_solver_setup/4, eplex_solve/1,
          eplex_get/2, eplex_var_get/3, optimize/2],
    desc:html("<P>
...
:- comment(optimize/2, [
    template:["optimize(+Objective, -Cost)","EplexInstance:optimize(+Objective, -Cost)"],
    args:["Objective":"Objective function: min(CostExpr) or max(CostExpr)",
        "Cost":"Output: cost of returned solution"],
    summary: "Setup problem, solve and instantiate problem variables",
    see_also:[($=)/2,($=<)/2,($>=)/2,($::)/2,integers/1,reals/1,
          eplex_solver_setup/1, eplex_solve/1,
          eplex_get/2, eplex_var_get/3, optimize/3],
    desc:html("<P>
    Take the problem defined by all the constraints that have been set up
    so far, and solve it for the given objective function.  If successful,
    instantiate all problem variables to their solution values.
