...
%               Kish Shen,       IC-Parc
% Version:	$Id: eplex_.ecl,v 1.21 2017/08/01 13:35:19 jschimpf Exp $
%
% TODO:
...
:- export flag_set/1, flag_create/1.
...
installed_solver(Solver, Version, File) :-
	...
	get_flag(system_object_suffix, SO),
	optimizer_code(Solver),
	concat_string(["se",Solver], ESolver),
	concat_string([ESolver,"*",SO], Pattern),
...
load_external_solver(Arch, Solver, Version, Suffix) :-
	( Solver = cplex ; Solver == gurobi ; Solver == glpk ), !,
...
:-
	...
	;
	    ...
	    get_flag(system_object_suffix, SO),
	    ( load_external_solver(Arch, Solver, Version, SO) -> true ; true ),
	    ( SO = ".o" ->
	...
	external(cplex_set_coeffs/2, p_cpx_set_coeffs(args)),
	...
	external(cplex_valid_method/1, p_cpx_valid_method(args)),
	external(cplex_valid_node_method/1, p_cpx_valid_node_method(args)),

        external(flag_create/1, p_flag_create(args)),
        external(flag_set/1, p_flag_set(args)),
        external(flag_reset/1, p_flag_reset(args)),
        external(cplex_link_flag/2, p_cpx_link_flag(args)),
...
lp_interval(Vs, Interval, Pool) :-
	...
	bound(L, Lo),
	bound(H, Hi).
    range([L..H], Lo, Hi) ?-  % for IC/FD compatibility
	bound(L, Lo),
	bound(H, Hi).
...
% may throw(abort) indicating error
extract_vars(Coll, Lo, Hi, CheckInt, VList0, VList) :-
	collection_to_list(flatten(Coll), List),
	(
	    foreach(V,List),
	    fromto(VList0,VList1,VList2,VList),
	    param(Lo,Hi,CheckInt)
	do
	    ( var(V) ->
		VList2 = [V|VList1]
	    ; number(V) ->
...
lp_add_vars(Handle, Xs) :-
        ...
        filter_and_index_new_vars(Handle, Xs, VarList, _OldXs, _OldCols, NAdded),
        setup_new_cols(Handle, [], [], [], [], NAdded, 0),
        cplex_flush_new_rowcols(Handle, 0),
...
lp_add_vars_interval(Handle, Xs, Lo, Hi) :-
        nonvar(Handle),
	Handle = prob{solver_id:SId, cplex_handle:CPH,option_vnames:VNames},
	filter_and_index_new_vars(Handle, Xs, VarList, OldVs, _OldCols, NAdded),
...
:- tool(optimize_pool/3, optimize_pool_body/4).
optimize_pool_body(OptExpr, ObjVal, Pool, Caller) :-
        optimize_pool_body(OptExpr, ObjVal, [], Pool, Caller).

:- tool(optimize_pool/4, optimize_pool_body/5).
optimize_pool_body(OptExpr, ObjValOrOptions, OptionsOrObjVal, Pool, Caller) :-
        get_pool_item(Pool, 0), % may fail
        !,
        ( nonvar(ObjValOrOptions), ObjValOrOptions=[_|_] ->
            % backward compatibility: swap arguments 2 and 3
            ObjVal=OptionsOrObjVal, Options=ObjValOrOptions
        ;
            ObjVal=ObjValOrOptions, Options=OptionsOrObjVal
        ),
        lp_demon_setup_body(OptExpr, ObjVal, [collect_from(pool(Pool))|Options], [], Handle, Caller),
        ...
        lp_cleanup(Handle).
optimize_pool_body(OptExpr, ObjVal, _Options, Pool, _) :-
        printf(error, "Eplex error: instance '%w' already has an associated solver in %w%n",
            [Pool,optimize(OptExpr, ObjVal)]),
        abort.
...
tolerable_range([], _, _, _, N, _, _) :- !, N=0.
...
low_level_setup(Handle, TempData, CstrNorm) :-
	Handle = prob{  vars:VarList,	        % in
        ...
        set_var_indices(VarList, Handle, 0, _, Cols),
        functor(ColCoeffs, '', Cols),
        process_constraints(CstrNorm, Rhs, BdCstrs, SId, 0, Rows, 0, NonZeros, ColCoeffs),
        get_flag(tmp_dir, TDir),
	...
	set_initial_bounds(CPH, SId, BdCstrs),	% may fail
        ...
        cplex_set_coeffs(CPH, ColCoeffs),
        set_sos(Handle, SOSs),			% may change prob type to MIP
        cplex_loadprob(CPH),
	cplex_link_flag(CPH, StopFlag),
...
lp_add_normalised(Handle, CstrNorm, BdCstrs, Integers, SOSs, IDCs, RowIdxs, TypeChanges, ChangedCols) :-
	...
	filter_and_index_new_vars(Handle, Vars, VarList, _, _OldCols, AddedCols),
	...
	setup_new_cols(Handle, [], [], [], [], AddedCols, 0),
	% set bounds on new variables
...
lp_add_columns(Handle, VarCols) :-
        ...
        setup_new_cols(Handle, NewObjCoeffs, NewLos, NewHis, NewColCoeffs, AddedCols, NonZeros),
...
setup_new_cols(Handle, NewObjCoeffs, NewLos, NewHis, NewColCoeffs, AddedCols, NonZeros) :-
        ...
        cplex_set_coeffs(CPH, NewColCoeffs).
...
lp_solve(Handle, ObjVal) :-
	Handle = prob{vars:VList,cplex_handle:CPH, timeout:TO,
                      ...
                      iis_rows:IISRows, stop_flag:StopFlag},
...
%	    writeln(log_output, 'loading basis'),
        ...
        ( var(StopFlag) -> true ; flag_reset(StopFlag) ),
...
process_option(stop_flag(StopFlag), Handle, _) ?-
        is_handle(StopFlag, flag),
        arg(stop_flag of prob, Handle, StopFlag),
        !.
...
lp_get1(prob{cplex_handle:CPH}, optimizer_param(Param), Value) ?- 
        atom(Param), 
        cplex_get_param(CPH, Param, V), !, Value=V.
...
lp_get1(prob{stop_flag:StopFlag}, stop_flag, Value) :- !,
        nonvar(StopFlag), Value = StopFlag.
lp_get1(Handle, What, Value) :-
...
lp_set1(Handle, method, M) :-
	cplex_method_codes(M, Code, AuxCode), !,
	(cplex_valid_method(Code) -> true
	; printf(error, "method %w not supported by this solver.%n", [M]),
	  abort
	),
...
lp_set1(Handle, node_method, M) :-
	cplex_method_codes(M, Code, AuxCode), !,
	(cplex_valid_node_method(Code) -> true
	; printf(error, "node method %w not supported by this solver.%n", [M]),
	  abort
	),
...
%lp_set1(Handle, stop_flag, Name) :- ...
%       Must be done at lp_setup time, to make sure the stop_flag is at least
%	as old as the problem (to avoid dangling references to the flag).
...
lp_get(has_sos, Value) ?-               !, cplex_get_param(0, -6, Value).
...
lp_get(optimizer_param(Param), Value) ?-
	atom(Param), 
	cplex_get_param(0, Param, V), !, Value=V.
lp_get(Param, Value) :-
...
lp_read_body(File, Format, Handle, Caller) :-
	...
	set_var_indices(VList, Handle, 0, NCols, _Cols),
...
retrieve_constraints(Handle, Rows, Constraints) :-
	Handle = prob{cplex_handle:CPH,vars:VList},
        ...
        (for(I,0,Rows-1), param(VArr,CPH,CType),
...
lp_verify_solution(Handle, VCs, VVs) :-
        ...
        cplex_get_prob_param(CPH, 0, Rows),
        VArr =.. [''|VList], % need random access to variables
        ( for(I,0,Rows-1), param(VArr,CPH,Tol,Handle), 
        ...
        ( foreacharg(V, VArr, I), param(CPH, Cols, Tol, IntTol, SolArr), 
          fromto(VVs, VV0,VV1, []) 
        do
            VIdx is Cols - I, %Vars are in reverse column order
...
% correspondence between linear and quad. problems
...
create_eplex_pool(Pool) :-
	create_constraint_pool(Pool, property(arity) of constraint_type, [
	    ...
	    eplex_cleanup/0 -> eplex_cleanup/1,
            optimize/2 -> optimize_pool/3,
            optimize/3 -> optimize_pool/4
	]).
