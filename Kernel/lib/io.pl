...
% Version:	$Id: io.pl,v 1.27 2017/08/05 00:16:30 jschimpf Exp $
% ----------------------------------------------------------------------
...
read_term_(Stream, Term, Options, Module) :-
    ...
    check_read_option(singletons(_), VarFlag, EF, EF) :- !,
    	VarFlag = 2'100.	% VARLIST_PLEASE
...
options_to_format(_, _, _, _, _, _, _, _, _, _, _) :-
	...
	set_bip_error(4).
    option_to_format(Junk, _, _, _, _, _) :- atomic(Junk), !,
	set_bip_error(5).
    option_to_format(precedence(P0), 0, 0, _D, P, _VN) :- !,
	( var(P0) -> set_bip_error(4) ; P = P0 ).
	...
	( arg(1,Option,Arg), var(Arg) ->
	    option_format(Option, C, S, D), !, set_bip_error(4)
	;
	    option_format(Option, C, S, D), !
	).
    option_to_format(Option, C, S, D, _P, _VN) :-
	( arg(1,Option,Arg), var(Arg) ->
	    option_format_compat(Option, C, S, D), !, set_bip_error(4)
	;
