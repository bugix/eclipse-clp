...
_write_exdr(stream_id nst, pword *pw, t_heap_htable *strhm, int *perr)
{
...
#if (SIZEOF_WORD > 4)
	    if ((int32_t) pw->val.nint != pw->val.nint)	/* need 'J' format */
	    {
		int32_t lo, hi;
		Store_Byte('J');
		lo = (int32_t) pw->val.nint;
		hi = (int32_t) (pw->val.nint >> 32);
		Store_Int32(hi);
		Store_Int32(lo);
	...
	case TBIG:
	{
	    int32_t *limbs = (int32_t*) BufferStart(pw->val.ptr);
	    int32_t lo, hi;
...
_read_exdr(ec_eng_t *ec_eng, stream_id nst, t_heap_htable *strhm, pword *pw)
{
	...
	case 'J':
	{
	    ...
	    Load_Int32(lo);
#if (SIZEOF_WORD >= 8)
	    Make_Integer(pw, ((word) hi << 32) + (uint32_t) lo);
#else
	    arg = TG;
	    Push_Buffer(8);
	    ...
	    ((int32_t *) BufferStart(arg))[0] = lo;
	    if (hi)		/* need two limbs */
	    {
		((int32_t *) BufferStart(arg))[1] = hi;
	    }
...
void Winapi
ec_int32_xdr(int32_t *l, char *dest)
{
	Store_Int32(*l);
}

void Winapi
ec_xdr_int32(char *buf, int32_t *l)
{
	Load_Int32(*l);
}
